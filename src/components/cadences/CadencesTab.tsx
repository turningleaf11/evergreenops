...
import { CadencePeek, cadenceScheduleLabel, cadenceNextDue } from "./CadencePeek";
...
export function CadencesTab() {
  ...
  const isCompletedToday = (cid: string): boolean => {
    const today = format(startOfDay(new Date()), "yyyy-MM-dd");
    const list = runsByCadence.get(cid) ?? [];
    return list.some(r => r.due_date === today && r.status === "completed");
  };

  const filtered = useMemo(() => {
    const base = filterDept === "all" ? cadences : cadences.filter(c => c.department_id === filterDept);
    return base;
  }, [cadences, filterDept]);
  ...
  return (
    <div className="space-y-6">
      ...
      <CadencePeek
        ...
        docs={docs}
        open={!!peekCadence}
        onOpenChange={(o) => { if (!o) setPeekCadence(null); }}
