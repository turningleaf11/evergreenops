...
export function TransactionTeamMembersPanel({
  transactionId,
  canManage,
  currentUserId,
}: {
  ...
  const add = async (userId: string) => {
    const { error } = await supabase.from("transaction_team_members").insert({
  ...
  const remove = async (id: string) => {
    const { error } = await supabase.from("transaction_team_members").delete().eq("id", id);
    if (error) {
      toast({ title: "Couldn't remove", description: error.message, variant: "destructive" });
      return;
    }
