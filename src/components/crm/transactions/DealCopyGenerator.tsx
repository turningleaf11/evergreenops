...
import { Sparkles, Loader2, Copy, Check, ArrowDownToLine } from "lucide-react";
...
export function DealCopyGenerator({
  ...
  // Spanish defaults on for Florida deals; user can still toggle it.
  useEffect(() => {
  ...
  async function generate() {
    ...
    setCopy((data as { copy: Copy }).copy);
  }
    ...
    <section className="space-y-3">
      ...
      {!copy && (
        <p className="text-[11px] text-muted-foreground">
          Drafts from the deal's facts, financing, and highlight. Review before sending — nothing is saved until you apply it.
        </p>
      )}
      ...
      {copy && (
        <div className="space-y-4">
          <CopyBlock
            ...
            text={copy.description_en}
            onUse={onUseAsDetails}
          />
          {spanish && (
            <CopyBlock
              label="Listing description (Spanish)"
              text={copy.description_es}
              onUse={onUseAsDetails}
            />
          )}
          <EmailBlock label="Buyer email" subject={copy.email_subject_en} body={copy.email_body_en} />
          {spanish && (
            <EmailBlock label="Buyer email (Spanish)" subject={copy.email_subject_es} body={copy.email_body_es} />
          )}
        </div>
      )}
    </section>
