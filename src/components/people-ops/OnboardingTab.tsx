...
export function OnboardingTab({ employeeId }: { employeeId: string }) {
  ...
  const completedCount = onboardingSteps.filter((s) => completedSet.has(s.id)).length;
  const pct = Math.round((completedCount / onboardingSteps.length) * 100);
    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-sm text-muted-foreground">{pct}% · {completedCount}/{onboardingSteps.length}</span>
        </div>
