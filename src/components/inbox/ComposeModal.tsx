...
function applyMergeFields(text: string, recipient: string): string {
  // Best-effort first-name extraction from the To field
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl p-0 gap-0">
        ...
        {/* Quick actions row */}
        <div className="px-5 py-2 flex items-center gap-1 flex-wrap border-b border-border/40">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              <DropdownMenuItem
                onClick={() => setBody((b) => b + " {{first_name}}")}
              >
                {"{{first_name}}"}
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => setBody((b) => b + " {{email}}")}
              >
                {"{{email}}"}
              </DropdownMenuItem>
            </DropdownMenuContent>
        ...
        {attachments.length > 0 && (
          <div className="px-5 py-2 border-t border-border/40 flex flex-wrap gap-1.5">
            {attachments.map((a, i) => (
              <div
                key={i}
              ...
              >
                ...
                <button
                  onClick={() =>
                    setAttachments((att) => att.filter((_, j) => j !== i))
                  }
