-- ============================================================================
-- Sync feature: 2-way comms layer between CEO and leadership.
-- Channels → Threads → Messages, with workspace-level customizable Tags.
-- Replaces the old strategy_items/directives + upward_proposals split with a
-- single conversational model that flows both directions.
-- ============================================================================
...
-- ── Tags (workspace-level, customizable) ───────────────────────────────────
...
-- SECURITY DEFINER helper so RLS policies on the members table don't recurse.
...
CREATE POLICY "sync_channels_insert_self" ON public.sync_channels
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = created_by);

DROP POLICY IF EXISTS "sync_channels_admin_all" ON public.sync_channels;
CREATE POLICY "sync_channels_admin_all" ON public.sync_channels
  FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));

-- sync_channel_members
DROP POLICY IF EXISTS "sync_members_select" ON public.sync_channel_members;
CREATE POLICY "sync_members_select" ON public.sync_channel_members
  FOR SELECT TO authenticated
  USING (public.is_sync_member(channel_id, auth.uid()));

DROP POLICY IF EXISTS "sync_members_insert" ON public.sync_channel_members;
CREATE POLICY "sync_members_insert" ON public.sync_channel_members
...
CREATE POLICY "sync_members_admin_all" ON public.sync_channel_members
  FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));

-- sync_threads
DROP POLICY IF EXISTS "sync_threads_select" ON public.sync_threads;
CREATE POLICY "sync_threads_select" ON public.sync_threads
  FOR SELECT TO authenticated
  USING (public.is_sync_member(channel_id, auth.uid()));

DROP POLICY IF EXISTS "sync_threads_insert" ON public.sync_threads;
CREATE POLICY "sync_threads_insert" ON public.sync_threads
  FOR INSERT TO authenticated
  WITH CHECK (public.is_sync_member(channel_id, auth.uid()) AND auth.uid() = author_id);

DROP POLICY IF EXISTS "sync_threads_update_members" ON public.sync_threads;
CREATE POLICY "sync_threads_update_members" ON public.sync_threads
  FOR UPDATE TO authenticated
  USING (public.is_sync_member(channel_id, auth.uid()));

DROP POLICY IF EXISTS "sync_threads_delete" ON public.sync_threads;
CREATE POLICY "sync_threads_delete" ON public.sync_threads
  FOR DELETE TO authenticated
  USING (auth.uid() = author_id OR public.has_role(auth.uid(), 'admin'));

-- sync_thread_messages
DROP POLICY IF EXISTS "sync_messages_select" ON public.sync_thread_messages;
CREATE POLICY "sync_messages_select" ON public.sync_thread_messages
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.sync_threads t
    WHERE t.id = thread_id AND public.is_sync_member(t.channel_id, auth.uid())
  ));

DROP POLICY IF EXISTS "sync_messages_insert" ON public.sync_thread_messages;
CREATE POLICY "sync_messages_insert" ON public.sync_thread_messages
  FOR INSERT TO authenticated
  WITH CHECK (
...
CREATE POLICY "sync_tags_admin_all" ON public.sync_tags
  FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));

-- ── Triggers ───────────────────────────────────────────────────────────────
DROP TRIGGER IF EXISTS sync_channels_set_updated ON public.sync_channels;
CREATE TRIGGER sync_channels_set_updated BEFORE UPDATE ON public.sync_channels
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS sync_threads_set_updated ON public.sync_threads;
CREATE TRIGGER sync_threads_set_updated BEFORE UPDATE ON public.sync_threads
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE FUNCTION public.sync_bump_thread_activity()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  UPDATE public.sync_threads
...
CREATE TRIGGER sync_bump_thread_activity_trg AFTER INSERT ON public.sync_thread_messages
  FOR EACH ROW EXECUTE FUNCTION public.sync_bump_thread_activity();

-- ============================================================================
-- Seed channels: a Direct channel between the CEO and every leader/admin,
-- plus a single #all-leadership group channel containing everyone.
-- Idempotent — skips if the all-leadership channel already exists.
-- ============================================================================
DO $$
DECLARE
  ceo_id UUID;
  ...
  all_lead_id UUID;
BEGIN
  SELECT user_id INTO ceo_id
  FROM public.user_roles
  WHERE role = 'admin' AND is_primary = true
  LIMIT 1;

  IF ceo_id IS NULL THEN
    RAISE NOTICE 'sync_feature seed: no primary admin — skipping channel seed';
    RETURN;
  END IF;

  SELECT id INTO all_lead_id FROM public.sync_channels
  WHERE type = 'group' AND name = 'All Leadership' LIMIT 1;

  IF all_lead_id IS NULL THEN
    INSERT INTO public.sync_channels (type, name, created_by)
    VALUES ('group', 'All Leadership', ceo_id)
    RETURNING id INTO all_lead_id;
  END IF;

  INSERT INTO public.sync_channel_members (channel_id, user_id) VALUES (all_lead_id, ceo_id)
  ...
  FOR leader IN
    ...
    AND p.user_id <> ceo_id
  LOOP
    -- Direct channel only if one doesn't already exist between these two
    IF NOT EXISTS (
      SELECT 1
      ...
      INSERT INTO public.sync_channels (type, created_by)
      VALUES ('direct', ceo_id)
      RETURNING id INTO new_channel_id;

      INSERT INTO public.sync_channel_members (channel_id, user_id) VALUES
        (new_channel_id, ceo_id),
        (new_channel_id, leader.user_id);
    END IF;

    -- Add to All Leadership
    INSERT INTO public.sync_channel_members (channel_id, user_id) VALUES (all_lead_id, leader.user_id)
    ON CONFLICT DO NOTHING;
  END LOOP;
END $$;
