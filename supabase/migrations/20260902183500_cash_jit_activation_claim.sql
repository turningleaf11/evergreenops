-- JIT Cash SFR activation claim.
--
-- The stage webhook now records activation intent. Cash materializes durable work
-- only after the Gateway has revalidated the opportunity against live HighLevel.
-- A short work-item lease prevents concurrent Cash sessions from receiving the
-- same active underwriting job.
