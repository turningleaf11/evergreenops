...
-- Loosen ai_logs.category so we can record full task lifecycle + future event types
...
INSERT INTO public.ai_logs (task_id, agent_id, agent_name, agent_emoji, category, message, created_at)
SELECT t.id, NULL, COALESCE(t.assigned_to, 'Agent'), '🤖', 'task_created', COALESCE(t.title, 'Task created'), t.created_at
FROM public.agent_tasks t
WHERE NOT EXISTS (SELECT 1 FROM public.ai_logs l WHERE l.task_id = t.id AND l.category = 'task_created');;
