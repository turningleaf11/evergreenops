...
CREATE POLICY "Workspace members can view lead files"
  ON public.lead_files FOR SELECT TO authenticated
  USING (workspace_id = public.get_user_workspace_id());

CREATE POLICY "Workspace members can add lead files"
  ON public.lead_files FOR INSERT TO authenticated
  WITH CHECK (workspace_id = public.get_user_workspace_id() AND uploaded_by = auth.uid());

CREATE POLICY "Workspace members can delete lead files"
