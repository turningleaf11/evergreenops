...
DROP POLICY IF EXISTS "Workspace can view custom fields" ON public.crm_custom_fields;
CREATE POLICY "Workspace can view custom fields" ON public.crm_custom_fields FOR SELECT TO authenticated USING (workspace_id = public.get_user_workspace_id());
DROP POLICY IF EXISTS "Admins manage custom fields" ON public.crm_custom_fields;
CREATE POLICY "Admins manage custom fields" ON public.crm_custom_fields FOR ALL TO authenticated
...
DROP TRIGGER IF EXISTS trg_crm_cf_updated ON public.crm_custom_fields;
CREATE TRIGGER trg_crm_cf_updated BEFORE UPDATE ON public.crm_custom_fields FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
