...
begin
  loop
    ...
    -- Resume an already-active SFR job first. This preserves restart safety.
    ...
    -- If a later authenticated OpportunityStageUpdate moved the opportunity
    -- away from the exact activation pipeline/stage, do not let Cash underwrite
    -- stale work. The work envelope remains durable and can be reopened by a
    -- later valid activation event.
    ...
    -- Manual-GHL items have no candidate fallback. If the Gateway has already
    -- rejected the opportunity-source CashValue as non-SFR / wrong SFR route for
    -- this activation, quarantine it instead of resuming the same failure every
    -- hour. A successful CashValue step always wins over a prior failed attempt.
