...
/** Existing HighLevel opportunity fields only. Ema must never create new CRM fields or dropdown values. */
const FIELD_IDS = {
...
const DEDICATED_FACT_KEYS = new Set([
  ...
  'asking_price','askingPrice','price','occupancy','occupancy_status',
  'mortgage_status','mortgageStatus','mortgage_balance','mortgageBalance',
  'back_payments','arrears','backPayments','piti','PITI','timeline','seller_timeline',
  'motivation','seller_motivation','condition','property_condition','hoa','has_hoa','hoa_exists',
  'hoa_amount','hoaAmount','hoa_restrictions','hoaRestrictions','hoa_frequency','hoa_duration','hoaFrequency',
  'listed_with_realtor','listed','listing_status','flood_and_utilities_info','flood_utilities',
  'deal_type','dealType','photos_zillow','photos_url','zillow_url','deal_room_url','criteria_met','criteriaMet',
]);

export class DealIntakeError extends Error {
  constructor(public status:number, public code:string) { super(code); this.name='DealIntakeError'; }
}

interface CandidateRow {
  id:string; workspace_id:string; ema_message_id:string; normalized_address:string|null;
  extracted_facts:Record<string,unknown>; evidence:Record<string,unknown>; source_type:string|null;
  intake_result:string|null; cash_screen_result:string; buy_box_fit_result:string;
  buy_box_fit_details:Record<string,unknown>; ghl_readiness:string; ghl_contact_id:string|null;
  ghl_opportunity_id:string|null; processing_status:string; is_test:boolean;
}
interface MessageRow {
  id:string; gmail_account:string|null; gmail_message_id:string; gmail_thread_id:string;
  sender_email:string|null; sender_name:string|null; subject:string|null; raw_metadata:Record<string,unknown>;
}
interface Route { propertyType:PropertyTypeOption; pipelineId:string; stageId:string }
interface OperationRow { id:string; operation_status:string; external_id:string|null }

export async function intakeCandidateToCrm(
  admin:SupabaseClient, ghl:GhlContext, workspaceId:string, candidateId:string,
  ...
  if (message.gmail_account) {
    try {
      sourceDocuments=await attachOriginalPdfDocuments(
        admin, ghl, workspaceId, candidate.id,
        {gmailAccount:message.gmail_account,gmailMessageId:message.gmail_message_id},
        opportunity.id,
      );
    } catch (error) {
      if (error instanceof SourceDocumentError) {
        sourceDocuments={status:'failed',error_code:error.code};
      } else {
        throw error;
      }
    }
  } else {
    sourceDocuments={status:'skipped',error_code:'gmail_account_missing'};
  }

  const {error}=await admin.from('ema_candidates').update({
    ghl_contact_id:contact.id,ghl_opportunity_id:opportunity.id,ghl_readiness:'ready',
    processing_status:'completed',last_evaluated_at:new Date().toISOString(),
  }).eq('id',candidate.id).eq('workspace_id',workspaceId);
  if(error)throw new DealIntakeError(500,'candidate_persist_failed');

  const qualificationSource=isEmaCrmIntakeEligible(candidate.buy_box_fit_result)?'ema_buy_box_fit':'cash_screen';
  return {
    candidate_id:candidate.id,
    qualification:{
      source:qualificationSource,buy_box_fit_result:candidate.buy_box_fit_result,
...
async function loadMessage(admin:SupabaseClient,w:string,id:string):Promise<MessageRow>{
  const{data,error}=await admin.from('ema_messages').select(
    'id, gmail_account, gmail_message_id, gmail_thread_id, sender_email, sender_name, subject, raw_metadata',
  ).eq('id',id).eq('workspace_id',w).maybeSingle();
...
function validateCandidate(c:CandidateRow):void{
  ...
  const emaCrmEligible=isEmaCrmIntakeEligible(c.buy_box_fit_result),cashQualified=['pass','marginal'].includes(c.cash_screen_result);
  if(!emaCrmEligible&&!cashQualified)throw new DealIntakeError(409,'deal_qualification_required');
  if(['excluded','needs_classification','needs_info'].includes(c.intake_result??'')||c.ghl_readiness==='excluded')throw new DealIntakeError(409,'candidate_not_intake_eligible');
  if(!looksLikeResolvedAddress(c.normalized_address?.trim()??''))throw new DealIntakeError(409,'normalized_address_required');
}
export function isEmaCrmIntakeEligible(result:string):boolean{return result==='fit'||result==='needs_info'}
...
/** Converts normalized values into the exact pre-existing HighLevel Property Type dropdown vocabulary. */
export function deriveRoute(f:Record<string,unknown>):Route{
  const raw=cleanString(firstValue(f,['property_type','propertyType','asset_class','assetClass','type']),120)?.toLowerCase()??'';
  const units=numberValue(firstValue(f,['units','unit_count','unitCount','number_of_units','site_count','pad_count']));
  if(/\brv\s*park\b|recreational vehicle park/.test(raw))return portfolioRoute('RV Park');
  if(/mobile\s*home\s*park|\bmhp\b/.test(raw))return portfolioRoute('Mobile Home Park');
  if(/town\s*house|townhome/.test(raw))return sfrRoute('Townhouse');
...
function portfolioRoute(propertyType:PropertyTypeOption):Route{return{propertyType,pipelineId:PORTFOLIO_PIPELINE_ID,stageId:PORTFOLIO_STAGE_ID}}

async function resolveContact(admin:SupabaseClient,ghl:GhlContext,c:CandidateRow,m:MessageRow):Promise<{id:string;disposition:'matched'|'created'|'persisted'}>{
  if(c.ghl_contact_id){const x=await getGhlContact(ghl,c.ghl_contact_id);if(!x.id)throw new DealIntakeError(409,'persisted_contact_invalid');return{id:x.id,disposition:'persisted'}}
  const email=cleanString(m.sender_email,320)?.toLowerCase()??null;if(!email)throw new DealIntakeError(409,'sender_identity_required');
  const es=await searchGhlContacts(ghl,{query:email,page:1,limit:20}),em=recordArray(es.contacts).filter(r=>cleanString(r.email,320)?.toLowerCase()===email);
  if(em.length>1)throw new DealIntakeError(409,'contact_match_ambiguous');if(em.length===1)return{id:requiredId(em[0].id),disposition:'matched'};
  const phone=cleanString(firstValue(c.extracted_facts,['sender_phone','phone','broker_phone']),64);
  if(phone){const ps=await searchGhlContacts(ghl,{query:phone,page:1,limit:20}),normalized=normalizePhone(phone),pm=recordArray(ps.contacts).filter(r=>normalizePhone(cleanString(r.phone,64)??'')===normalized);if(pm.length>1)throw new DealIntakeError(409,'contact_match_ambiguous');if(pm.length===1)return{id:requiredId(pm[0].id),disposition:'matched'}}
  const key=`ema:${c.id}:contact:v1`,prior=await getOperation(admin,c.workspace_id,key);
  if(prior?.operation_status==='succeeded'&&prior.external_id)return{id:prior.external_id,disposition:'persisted'};
  if(prior&&['executing','needs_reconciliation'].includes(prior.operation_status))throw new DealIntakeError(409,'contact_create_requires_reconciliation');
...
async function resolveOpportunity(admin:SupabaseClient,ghl:GhlContext,c:CandidateRow,address:string,route:Route,contactId:string,customFields:Array<Record<string,unknown>>):Promise<{id:string;created:boolean;pipelineId:string;stageId:string}>{
  ...
  const key=`ema:${c.id}:opportunity:v1`,prior=await getOperation(admin,c.workspace_id,key);
  if(prior?.operation_status==='succeeded'&&prior.external_id){const x=await getGhlOpportunity(ghl,prior.external_id);return{id:prior.external_id,created:false,pipelineId:requiredId(x.pipeline_id),stageId:requiredId(x.stage_id)}}
  if(prior&&['executing','needs_reconciliation'].includes(prior.operation_status))throw new DealIntakeError(409,'opportunity_create_requires_reconciliation');
...
async function ensureIntakeNote(admin:SupabaseClient,ghl:GhlContext,c:CandidateRow,m:MessageRow,address:string,contactId:string,oppId:string,created:boolean):Promise<{id:string;disposition:'created'|'reconciled'|'persisted'}>{
  ...
  if(prior?.operation_status==='succeeded'&&prior.external_id)return{id:prior.external_id,disposition:'persisted'};
  const notes=await listGhlContactNotes(ghl,contactId),marker=`Intake Ref: ${c.id}`,existing=recordArray(notes.notes).find(r=>cleanString(r.body,5000)?.includes(marker));
  if(existing){const id=requiredId(existing.id);if(prior)await finishOperation(admin,prior.id,id,{reconciled:true});return{id,disposition:'reconciled'}}
  if(prior&&['executing','needs_reconciliation'].includes(prior.operation_status))throw new DealIntakeError(409,'note_create_requires_reconciliation');
...
export function buildOpportunityCustomFields(
  ...
  const mappings:Array<[string,string[]]>=[
    [FIELD_IDS.deal_type,['deal_type','dealType']],[FIELD_IDS.photos_zillow,['photos_zillow','photos_url','zillow_url','deal_room_url']],
    [FIELD_IDS.listed,['listed_with_realtor','listed','listing_status']],[FIELD_IDS.condition,['condition','property_condition']],
    [FIELD_IDS.motivation,['motivation','seller_motivation']],[FIELD_IDS.asking_price,['asking_price','askingPrice','price']],
    [FIELD_IDS.timeline,['timeline','seller_timeline']],[FIELD_IDS.occupancy,['occupancy','occupancy_status']],
    [FIELD_IDS.hoa,['hoa','has_hoa','hoa_exists']],[FIELD_IDS.hoa_amount,['hoa_amount','hoaAmount']],
    [FIELD_IDS.hoa_duration,['hoa_frequency','hoa_duration','hoaFrequency']],[FIELD_IDS.hoa_restrictions,['hoa_restrictions','hoaRestrictions']],
    [FIELD_IDS.mortgage_status,['mortgage_status','mortgageStatus']],[FIELD_IDS.mortgage_balance,['mortgage_balance','mortgageBalance']],
    [FIELD_IDS.piti,['piti','PITI']],[FIELD_IDS.arrears,['back_payments','arrears','backPayments']],
...
export function buildDealDetails(facts:Record<string,unknown>,evidence:Record<string,unknown>={}):string|null{
  const lines:string[]=[],skipKeys=new Set(['arv_range_low','arv_range_high']);
  const arvLow=numberValue(firstValue(facts,['arv_range_low','arv_low'])),arvHigh=numberValue(firstValue(facts,['arv_range_high','arv_high']));
  for(const[key,value]of Object.entries(facts)){if(DEDICATED_FACT_KEYS.has(key)||skipKeys.has(key))continue;const rendered=formatDetailValue(key,value);if(rendered!==null)lines.push(`${humanizeKey(key)}: ${rendered}`)}
  if(arvLow!==null||arvHigh!==null){const low=arvLow!==null?formatCurrency(arvLow):'Unknown',high=arvHigh!==null?formatCurrency(arvHigh):'Unknown';lines.push(`ARV Range: ${low} - ${high}`)}
...
function buildNoteBody(c:CandidateRow,m:MessageRow,address:string,oppId:string,event:string):string{
  ...
  if(c.buy_box_fit_result==='fit'){const verification=recordString(c.buy_box_fit_details,'verification_status')??'provisional';lines.push(`Preliminary Buy-Box Fit: FIT (${verification.toUpperCase()})`)}
  else if(c.buy_box_fit_result==='needs_info'){lines.push('Preliminary Buy-Box Fit: NEEDS INFO (TEAM REVIEW)');const missing=buyBoxUnknownFields(c.buy_box_fit_details);if(missing.length)lines.push(`Missing/Unknown: ${missing.join(', ')}`)}
  else if(['pass','marginal'].includes(c.cash_screen_result))lines.push(`Cash Screen: ${c.cash_screen_result.toUpperCase()}`);
  lines.push('Source: Email');if(m.sender_name)lines.push(`Sender: ${m.sender_name.slice(0,200)}`);if(m.sender_email)lines.push(`Sender Email: ${m.sender_email.slice(0,320)}`);
  for(const[label,keys]of[['Asking Price',['asking_price','askingPrice','price']],['Property Type',['property_type','propertyType','asset_class']],['Occupancy',['occupancy','occupancy_status']]]as Array<[string,string[]]>){const value=scalarString(firstValue(f,keys),160);if(value)lines.push(`${label}: ${value}`)}
  lines.push('',`Opportunity ID: ${oppId}`,`Gmail Message: ${m.gmail_message_id.slice(0,300)}`,`Intake Ref: ${c.id}`);return lines.join('\n').slice(0,5000);
}
function buyBoxUnknownFields(details:Record<string,unknown>):string[]{return recordArray(details.unknown).flatMap(r=>{const field=cleanString(r.field,120);return field?[field]:[]})}
...
function firstValue(record:Record<string,unknown>,keys:string[]):unknown{for(const key of keys){const value=record[key];if(value!==undefined&&value!==null&&value!=='')return value}return null}
function scalarString(value:unknown,max:number):string|null{if(typeof value==='string'){const text=value.trim();return text?text.slice(0,max):null}if(typeof value==='number'&&Number.isFinite(value))return String(value).slice(0,max);if(typeof value==='boolean')return value?'Yes':'No';return null}
function cleanString(value:unknown,max:number):string|null{return scalarString(value,max)}
function numberValue(value:unknown):number|null{if(typeof value==='number'&&Number.isFinite(value))return value;if(typeof value==='string'){const parsed=Number(value.replace(/[^0-9.-]/g,''));return Number.isFinite(parsed)?parsed:null}return null}
function formatDetailValue(key:string,value:unknown):string|null{if(value===null||value===undefined||value==='')return null;if(typeof value==='number'&&Number.isFinite(value)){if(/(price|arv|rent|balance|piti|amount|repairs|noi|income|expense)/i.test(key))return formatCurrency(value);return value.toLocaleString('en-US')}if(typeof value==='boolean')return value?'Yes':'No';if(typeof value==='string')return value.trim().slice(0,1000)||null;if(Array.isArray(value)){const values=value.flatMap(item=>{const rendered=scalarString(item,300);return rendered?[rendered]:[]});return values.length?values.join(', ').slice(0,1000):null}return null}
function formatCurrency(value:number):string{return`$${Math.round(value).toLocaleString('en-US')}`}
function humanizeKey(key:string):string{const known:Record<string,string>={sqft:'Square Feet',lot_size_sqft:'Lot Size',year_built:'Year Built',tenant_rent_monthly:'Tenant Rent / Month',arv:'Estimated ARV',email_arv:'Email ARV',email_asking_price:'Email Asking Price',bedrooms:'Bedrooms',bathrooms:'Bathrooms',units:'Units',sites:'Sites',pads:'Pads',zip:'ZIP'};return known[key]??key.replace(/_/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}
function recordArray(value:unknown):Array<Record<string,unknown>>{return Array.isArray(value)?value.filter((x):x is Record<string,unknown>=>typeof x==='object'&&x!==null&&!Array.isArray(x)):[]}
function recordAt(value:Record<string,unknown>,key:string):Record<string,unknown>{const candidate=value[key];return candidate&&typeof candidate==='object'&&!Array.isArray(candidate)?candidate as Record<string,unknown>: {}}
function requiredId(value:unknown):string{const id=cleanString(value,128);if(!id||!/^[A-Za-z0-9_-]+$/.test(id))throw new DealIntakeError(502,'invalid_ghl_identifier');return id}
function canonicalAddress(value:string):string{return value.toLowerCase().replace(/[^a-z0-9]/g,'')}
function normalizePhone(value:string):string{return value.replace(/\D/g,'').slice(-10)}
function firstName(name:string|null):string|null{const clean=cleanString(name,300);return clean?clean.split(/\s+/)[0].slice(0,100):null}
function lastName(name:string|null):string|null{const clean=cleanString(name,300);if(!clean)return null;const parts=clean.split(/\s+/);return parts.length>1?parts.slice(1).join(' ').slice(0,150):null}
function recordString(value:Record<string,unknown>,key:string):string|null{return typeof value[key]==='string'?String(value[key]).slice(0,160):null}