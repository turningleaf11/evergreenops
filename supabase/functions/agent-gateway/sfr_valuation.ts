...
export interface SfrValuationResult {
  ...
  comp_source:'rentcast'|'public_evidence'|'mixed'|'none';
  comps_found:number;
...
export async function runSfrOpportunityValuation(
  ...
  const{data:candidates,error}=await admin.from('ema_candidates').select('id').eq('workspace_id',workspaceId).eq('ghl_opportunity_id',opportunityId).limit(2);
  ...
  return runResolvedSfrValuation(admin,{targetSource:'ghl_opportunity',candidateId,opportunityId,subject},options,fetchImpl);
}

async function runResolvedSfrValuation(
  admin:SupabaseClient,
  target:{targetSource:'ema_candidate'|'ghl_opportunity';candidateId:string|null;opportunityId:string|null;subject:CashValueSubject},
  options:SfrValuationOptions,
  ...
  const notes:string[]=[];
  if(options.subject_evidence?.source_url)notes.push(`Public subject facts were supplied from ${options.subject_evidence.source_url}; the Gateway does not treat the source as a sold comp.`);

  const publicComps=dedupeComps((options.public_comps??[]).map(comp=>({...comp,source:comp.source??'public_evidence'})));
  const providerState:SfrValuationResult['providers']={
    ...
    public_evidence:{status:publicComps.length||options.subject_evidence?'used':'not_supplied',comp_count:publicComps.length,subject_evidence:Boolean(options.subject_evidence)},
  ...
  if(config.rentcastApiKey){
    try{
      ...
      providerState.rentcast={status:'used',comp_count:rentcastComps.length,avm:rentcast.avm,avm_range:rentcast.avmRange,error_code:null};
  ...
  const comps=dedupeComps([...rentcastComps,...publicComps]);
  const compSource:SfrValuationResult['comp_source']=rentcastComps.length&&publicComps.length?'mixed':rentcastComps.length?'rentcast':publicComps.length?'public_evidence':'none';
  const cashValue=calculateCashValue(subject,comps);
  if(publicComps.length)notes.push(`${publicComps.length} source-backed public comp${publicComps.length===1?' was':'s were'} supplied for CashValue review. Source URLs remain attached to the comp evidence for audit.`);
  if(cashValue.status==='thin_comp_set')notes.push(`Only ${cashValue.selected_comp_count} defensible comp${cashValue.selected_comp_count===1?'':'s'} were found. Cash must submit the available comps and low-confidence CashValue to the team rather than hiding the evidence.`);
  if(cashValue.selected_comp_count===0)notes.push('No defensible comps were found from the currently available provider/evidence set.');
...
export function subjectFromCandidate(candidate:Pick<CandidateRow,'normalized_address'|'extracted_facts'>):CashValueSubject{
  ...
  try{propertyType=deriveRoute(facts).propertyType}catch(error){if(error instanceof DealIntakeError)throw new SfrValuationError(409,'property_type_unresolved');throw error}
  if(propertyType!=='Single Family Residence')throw new SfrValuationError(409,'single_family_residence_required');
  const address=stringValue(candidate.normalized_address);if(!address)throw new SfrValuationError(409,'normalized_address_required');
  return{address,property_type:'Single Family Residence',sqft:numberValue(first(facts,['sqft','square_feet','squareFeet','living_area','livingArea']))??0,year_built:numberValue(first(facts,['year_built','yearBuilt','built_year','builtYear'])),beds:numberValue(first(facts,['bedrooms','beds','bed_count','bedCount'])),baths:numberValue(first(facts,['bathrooms','baths','bath_count','bathCount'])),stories:numberValue(first(facts,['stories','story_count','storyCount','floor_count','floorCount'])),build_style:stringValue(first(facts,['build_style','buildStyle','architecture_type','architectureType']))};
}

export function subjectFromOpportunityRecord(opportunity:Record<string,unknown>):CashValueSubject{
  const pipelineId=stringValue(opportunity.pipelineId??opportunity.pipeline_id);if(pipelineId!==SFR_PIPELINE_ID)throw new SfrValuationError(409,'sfr_pipeline_required');
  ...
  if(normalizePropertyType(propertyType??'')!=='Single Family Residence')throw new SfrValuationError(409,'single_family_residence_required');
  const address=stringValue(customFieldValue(fields,GHL_PROPERTY_ADDRESS_FIELD_ID))??stringValue(opportunity.name);if(!address)throw new SfrValuationError(409,'normalized_address_required');
...
async function subjectFromGhlOpportunity(admin:SupabaseClient,opportunityId:string,fetchImpl:typeof fetch):Promise<CashValueSubject>{
  ...
  const response=await fetchImpl(`${GHL_BASE}/opportunities/${encodeURIComponent(opportunityId)}`,{headers:{Authorization:`Bearer ${context.apiKey}`,Version:GHL_VERSION,Accept:'application/json'},signal:AbortSignal.timeout(20000)});
  ...
  const payload=record(await response.json()),opportunity=record(payload.opportunity);
...
export async function fetchRentCastValuation(
  ...
  const resolvedSubject:Partial<CashValueSubject>={sqft:numberValue(subjectRow.squareFootage)??subject.sqft,year_built:numberValue(subjectRow.yearBuilt)??subject.year_built,beds:numberValue(subjectRow.bedrooms)??subject.beds,baths:numberValue(subjectRow.bathrooms)??subject.baths,stories:numberValue(record(subjectRow.features).floorCount)??subject.stories,build_style:stringValue(record(subjectRow.features).architectureType)??subject.build_style};
  const sqft=Number(resolvedSubject.sqft??0);if(!(sqft>0))throw new SfrValuationError(409,'subject_sqft_required');
  const yearBuilt=numberValue(resolvedSubject.year_built),beds=numberValue(resolvedSubject.beds),baths=numberValue(resolvedSubject.baths),subjectLat=numberValue(subjectRow.latitude),subjectLng=numberValue(subjectRow.longitude),subjectId=stringValue(subjectRow.id);
  ...
  if(comps.length<3){const expandedParams=rentCastCompParams(subject.address,sqft,yearBuilt,beds,baths,365,20);const expandedRaw=await rentCastJson(`${RENTCAST_BASE}/properties?${expandedParams.toString()}`,apiKey,fetchImpl);comps=dedupeComps([...comps,...normalizeRentCastComps(expandedRaw,subject,subjectId,subjectLat,subjectLng)])}
  let avm:number|null=null,avmRange:{low:number;high:number}|null=null;
  try{const avmParams=new URLSearchParams({address:subject.address,propertyType:'Single Family',squareFootage:String(Math.round(sqft)),maxRadius:'1',daysOld:'365',compCount:'20',lookupSubjectAttributes:'true'});if(beds!==null)avmParams.set('bedrooms',String(beds));if(baths!==null)avmParams.set('bathrooms',String(baths));const avmRaw=record(await rentCastJson(`${RENTCAST_BASE}/avm/value?${avmParams.toString()}`,apiKey,fetchImpl));avm=numberValue(avmRaw.price);const low=numberValue(avmRaw.priceRangeLow),high=numberValue(avmRaw.priceRangeHigh);if(low!==null&&high!==null)avmRange={low,high}}catch{/* AVM is supporting evidence only. */}
...
function rentCastCompParams(address:string,sqft:number,yearBuilt:number|null,beds:number|null,baths:number|null,saleDateRange:number,yearTolerance:number):URLSearchParams{
  const params=new URLSearchParams({address,radius:'1',propertyType:'Single Family',squareFootage:`${Math.max(1,Math.round(sqft-250))}:${Math.round(sqft+250)}`,saleDateRange:String(saleDateRange),limit:String(MAX_SOURCE_RECORDS)});
  if(yearBuilt!==null)params.set('yearBuilt',`${Math.max(1600,Math.round(yearBuilt-yearTolerance))}:${Math.round(yearBuilt+yearTolerance)}`);if(beds!==null)params.set('bedrooms',`${Math.max(0,beds-1)}:${beds+1}`);if(baths!==null)params.set('bathrooms',`${Math.max(0,baths-1)}:${baths+1}`);return params;
}
...
function normalizeRentCastComps(raw:unknown,subject:CashValueSubject,subjectId:string|null,subjectLat:number|null,subjectLng:number|null):CashValueComp[]{
  const rows=Array.isArray(raw)?raw:[],comps:CashValueComp[]=[];
  for(const rawRow of rows){const row=record(rawRow),id=stringValue(row.id),address=stringValue(row.formattedAddress),salePrice=numberValue(row.lastSalePrice),saleDate=stringValue(row.lastSaleDate),squareFootage=numberValue(row.squareFootage);if(!address||salePrice===null||!saleDate||squareFootage===null||squareFootage<=0)continue;if((subjectId&&id===subjectId)||address.toLowerCase()===String(subject.address??'').toLowerCase())continue;const lat=numberValue(row.latitude),lng=numberValue(row.longitude),distance=subjectLat!==null&&subjectLng!==null&&lat!==null&&lng!==null?haversineMiles(subjectLat,subjectLng,lat,lng):null;if(distance===null||distance>1)continue;comps.push({id,address,property_type:mapRentCastPropertyType(stringValue(row.propertyType)),sqft:squareFootage,year_built:numberValue(row.yearBuilt),beds:numberValue(row.bedrooms),baths:numberValue(row.bathrooms),stories:numberValue(record(row.features).floorCount),build_style:stringValue(record(row.features).architectureType),condition:null,sale_price:salePrice,sale_date:saleDate.slice(0,10),distance_miles:Math.round(distance*100)/100,source:'rentcast_property_record'})}
  return comps;
}

function dedupeComps(comps:CashValueComp[]):CashValueComp[]{const seen=new Set<string>(),out:CashValueComp[]=[];for(const comp of comps){const key=(comp.id||comp.address).trim().toLowerCase();if(seen.has(key))continue;seen.add(key);out.push(comp)}return out}
async function rentCastJson(url:string,apiKey:string,fetchImpl:typeof fetch):Promise<unknown>{const response=await fetchImpl(url,{headers:{'X-Api-Key':apiKey,Accept:'application/json'},signal:AbortSignal.timeout(20000)});if(response.status===401||response.status===403)throw new SfrValuationError(503,'rentcast_auth_failed');if(response.status===429)throw new SfrValuationError(503,'rentcast_rate_limited');if(!response.ok)throw new SfrValuationError(502,'rentcast_request_failed');return response.json()}
function customFieldValue(fields:Record<string,unknown>[],fieldId:string):unknown{const field=fields.find(item=>stringValue(item.id)===fieldId);if(!field)return null;for(const key of ['fieldValueString','fieldValueNumber','fieldValueDate','fieldValueArray','value'])if(field[key]!==undefined&&field[key]!==null)return field[key];return null}
