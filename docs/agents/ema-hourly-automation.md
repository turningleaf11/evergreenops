...
openclaw automations create "every 1h" \
  ...
  --agent ema \
  --session isolated \
  --message "Run Ema's hourly inbox duty exactly as defined in the ema skill. Use a 2-hour Gmail lookback, rely on Gateway idempotency for overlap, process new deals and existing-deal updates, and never activate Cash. Return NO_REPLY when no human attention is required."
```

Delivery should be configured intentionally on the hosted instance:

- During burn-in, `--no-deliver` is acceptable while operators inspect run history.
- For normal production, route exception-only output to the operator/owner. The prompt must return `NO_REPLY` when the run is clean so hourly executions do not create noise.
...
8. For each new persisted candidate:
   ...
   - if `fit` or `needs_info`, call `deal_intake_to_crm`;
...
The Agent Gateway owns deduplication:
...
2. Create the hourly automation and record its job ID.
3. Disable/remove any older Ema inbox polling automation so exactly one recurring inbox job remains.
4. Force-run the job once.
5. Verify run history shows a successful isolated Ema turn.
6. Verify Gateway audit history shows `system.whoami`, email reads, and only the deal actions actually needed by the test message.
7. Repeat the forced run against the same lookback and confirm it does not create duplicate `ema_messages`, `ema_candidates`, contacts, opportunities, or notes.
8. Confirm the next natural run is approximately one hour later.
